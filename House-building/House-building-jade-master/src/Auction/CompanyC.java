...
public class CompanyC extends Agent {
    ...
    //CompanyC can commit for max 2 Build

    private int can_commit=0;

    // Building and Price of CompanyC

    String[] Building = {"SitePreparation", "Floors", "Walls", "Roof", "WindowsDoors", "Plumbing", "ElectricalSystem", "Painting"};
    int[] BuildingPrice = {1900,900,900,1100,2000,600,300,1100};
    int[] BuildCommit={0,0,0,0,0,0,0,0};
    ...
    private int sum (int[] BuildCommit)
    {int x=0;
        for(int i=0; i< BuildCommit.length;i++)
          x= x+ BuildCommit[i];
    ...
    private String evaluateAction(String cfp) {
        ...
        for(int i=0;i<8;i++)
        {
            if (itemName.equals(Building[i]))
                myPrice=BuildingPrice[i];

                if (currentBidder.equals(this.getAID().toString())) {
                    BuildCommit[i]=1;
                }
                else BuildCommit[i]=0;
        }

        can_commit= sum(BuildCommit);
        if((myPrice < itemPrice) && (!currentBidder.equals(this.getAID().toString())) && can_commit < 2)
        {can_commit++;
            return (itemName+","+myPrice);
        }
        else return "nothing";
        }
